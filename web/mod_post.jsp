...
<html>
 ...
 if(!RS_result.next())
{
ou.print("<script>alert('系统故障,请勿越权操作!');window.history.go(-1);</script>");
	}
else{
	...
	String pwd=RS_result.getString("pwd");
  	  ...
  	  if(pwd.equals(ymm))
  	  {
  	  	...
  	  	ou.print("<script>alert('修改成功!!');window.history.go(-1);</script>");
  	  }
  	  else
  	  {
  	  	ou.print("<script>alert('对不起,您的原密码不正确,请重试!');window.history.go(-1);</script>");
  	  }
	}
RS_result.close();
 %>
